...
int main(int argc, char** argv)
{
	try
	{
		...
		{
			if (_strcmpi(argv[i], "-image1") == 0 && i + 1 < argc)
				image_path1 = argv[++i];
			else if (_strcmpi(argv[i], "-image2") == 0 && i + 1 < argc)
				image_path2 = argv[++i];
			else if (_strcmpi(argv[i], "-algorithm") == 0 && i + 1 < argc)
				algo = argv[++i];
		}

		if (image_path1.empty() || image_path2.empty())
			throw std::exception("Invalid arguments: image1 and image2 are mandatory parameters.");


		cv::Ptr<cv::Feature2D> detector;

		if (algo.empty() || algo == "BRISK")
			detector = cv::BRISK::create();
		else if (algo == "ORB")
			detector = cv::ORB::create();
		else
			throw std::exception("Unknown algorithm has been specified: allowed algorithms are BRISK and ORB.");

		cv::Ptr<cv::DescriptorMatcher> matcher = DescriptorMatcher::create(DescriptorMatcher::MatcherType::BRUTEFORCE);
	...
	{
		std::cerr << "Image stitching failed with the following error message: " << e.what() << std::endl;
		return -1;
	}
	return 0;
}